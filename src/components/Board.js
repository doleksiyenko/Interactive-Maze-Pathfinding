...
class Board extends Component {
  ...
  createEmptyBoard = () => {
    ...
    for (let i = 0; i < numberOfNodes; i++) {
      ...
      if (
        i % elementsPerRow === 0 ||
        i % elementsPerRow === elementsPerRow - 1 ||
        Math.floor(i / elementsPerRow) === 0 ||
        Math.floor(i / elementsPerRow) === elementsPerRow - 1
      ) {
        newNode = [1, i % elementsPerRow, Math.floor(i / elementsPerRow)];
      } else {
        newNode = [0, i % elementsPerRow, Math.floor(i / elementsPerRow)];
      }
      ...
      <div className="board">
        {this.state.nodes.map((element) => (
          <Node
            ...
            mazeSize={this.state.elementsPerRow}
          ></Node>
        ))}
      </div>
    );
