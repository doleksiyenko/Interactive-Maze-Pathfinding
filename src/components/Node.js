...
class Node extends Component {
  ...
  changeWallPassage = () => {
    if (this.state.status === 1 || this.state.status === 0) {
      ...
      this.props.changeStatus((this.state.status + 1) % 2, this.props.position);
    }
  };
  ...
  createBorderWall = () => {
    if (
      ...
      this.state.position[0] === this.props.mazeSize - 1 ||
      this.state.position[1] === 0 ||
      this.state.position[1] === this.props.mazeSize - 1
    ) {
      this.setState({
        status: 1,
      });
      this.props.changeStatus(1, this.state.position);
    }
  };
  ...
  componentDidUpdate() {
    if (this.props.reset) {
      ...
      // then create a border wall around the board
